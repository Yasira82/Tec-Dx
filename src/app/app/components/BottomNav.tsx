...
import { Icon, type DxIconName } from './Icon';
import { useTranslation } from '@/lib/i18n';

export type DxTab = 'build' | 'capabilities' | 'guides' | 'settings';

export function BottomNav({ active, onSelect }: { active: DxTab; onSelect: (t: DxTab) => void }) {
  const { t } = useTranslation();
  const ITEMS: { key: DxTab; icon: DxIconName; label: string }[] = [
    { key: 'build',        icon: 'code',     label: t.dx.nav.build        },
    { key: 'capabilities', icon: 'grid',     label: t.dx.nav.capabilities },
    { key: 'guides',       icon: 'book',     label: t.dx.nav.guides       },
    { key: 'settings',     icon: 'settings', label: t.dx.nav.settings     },
  ];
  return (
    <nav style={{
