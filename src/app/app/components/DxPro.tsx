...
// DX Builder Pro — the monetization surface (C-115 §7: API access tiers /
// premium builder features). A real Pi User-to-App payment; also satisfies the
// Pi Portal "Process a Transaction" step. ADR-007 dual-mode: Hub navigation →
// Mode 1 (Hub modal); standalone → Mode 2 (createU2APayment). Approves under
// PI_API_KEY_DX (never the default Hub key — C-12 §11).
...
const MEMO    = 'TEC DX — Builder Pro (1 month)';
...
export function DxPro() {
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      setStatus('paying');
      const result = await createU2APayment(PRICE, MEMO, { item_id: ITEM_ID, plan: 'dx_pro' }, internalId);
  ...
  if (status === 'success') {
    return (
      <div style={{ ...card, borderColor: `${TEC_COLORS.success}66` }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: TEC_COLORS.success }}>✅ Builder Pro active</div>
        <div style={{ fontSize: 12, color: TEC_COLORS.subtext, marginTop: 6 }}>
          Payment received. Higher API limits + priority support unlocked 🛠️.
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={card}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 12 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: TEC_COLORS.gold }}>🛠️ DX Builder Pro</div>
      ...
      <div style={{ fontSize: 12, color: TEC_COLORS.subtext, marginTop: 8, lineHeight: 1.5 }}>
        ...
        new capabilities + templates. Capability certification stays with SYSTEM;
        DX distributes, it never certifies.
      </div>
